...
| Asset Protocol 全路径授权（`scope: ["**"]`） | 桌面笔记应用需加载用户选择的 workspace 中的图片资源，workspace 可位于任意路径（如 `E:\test-notes`）。仅允许 `$HOME` 会导致非 HOME 路径下的图片无法显示。风险可控：无远程入口，仅加载用户主动打开的 workspace 内资源 |
