...
pub(crate) fn collects_as_inline_text(tag: HtmlTag) -> bool {
    // Replaced elements (img, svg) carry binary payload, not text content.
    // They must NOT be collected as inline text runs — doing so silently
    // drops them (see flatten_element, which renders them as images/SVG).
    // Excluding them lets block layout route them through flatten_element.
    !matches!(tag, HtmlTag::Svg | HtmlTag::Img) && tag.is_inline()
}

/// True for replaced/atomic elements (`<img>`, `<svg>`) that are inline in the
/// HTML sense but must be laid out via [`super::engine::flatten_element`]
/// rather than text-run collection. Block containers treat such children as
/// "block-like" so they enter the mixed-content path that renders them.
pub(crate) fn is_replaced_layout_child(tag: HtmlTag) -> bool {
