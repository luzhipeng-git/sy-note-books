...
use super::helpers::{
    ...
    collects_as_inline_text, has_background_paint, heading_level, is_replaced_layout_child,
    ...
    let has_block_kids_for_wrapper = nesting_depth < 40
        ...
        && el.children.iter().any(|c| {
            matches!(c, DomNode::Element(e)
                if (e.tag.is_block() || is_replaced_layout_child(e.tag))
            ...
            && el.children.iter().any(|c| {
                matches!(c, DomNode::Element(e)
                    if (has_own_margins(e.tag)
                        ...
                        || is_replaced_layout_child(e.tag)
                ...
                match child {
                    ...
                    DomNode::Element(child_el)
                        if (child_el.tag.is_block()
                            || is_replaced_layout_child(child_el.tag)
