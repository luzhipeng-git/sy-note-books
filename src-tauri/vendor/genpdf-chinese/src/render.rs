...
impl<'p> Layer<'p> {
    ...
    fn add_polygon_shape<I>(&self, points: I, mode: printpdf::path::PaintMode)
    where
        I: IntoIterator<Item = LayerPosition>,
    {
        let polygon_points: Vec<_> = points
            .into_iter()
            .map(|pos| (self.transform_position(pos).into(), false))
            .collect();
...
impl<'p> Area<'p> {
    ...
    ///
    /// The position is relative to the upper left corner of the area.
    pub fn draw_filled_rect(&self, position: Position, size: Size, fill_color: Color) {
        if size.width <= Mm(0.0) || size.height <= Mm(0.0) {
            return;
        }

        let top_left = position;
        let top_right = Position::new(position.x + size.width, position.y);
        let bottom_right = Position::new(position.x + size.width, position.y + size.height);
        let bottom_left = Position::new(position.x, position.y + size.height);
