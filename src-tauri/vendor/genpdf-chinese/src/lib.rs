...
//!
//! `genpdfi` is a high-level PDF generator built ontop of [`printpdf`][] and [`ab_glyph`][].  It
...
// PATCHED (vendored): The original blanket `impl<T: Into<Mm>> From<T> for Margins`
// triggers E0119 on Rust 1.96+ due to a coherence conflict with the `time` crate's
// own `From` impls. Replaced with concrete impls so external callers passing an `Mm`
// still convert via `.into()`. Numeric callers should use `Margins::all(value)`.
