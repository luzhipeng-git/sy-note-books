...
//! from a file ([`FontData::load`][]) or from bytes ([`FontData::new`][]).  See the [`ab_glyph`][]
...
// `Font` must be in scope to call ab_glyph's font-metric methods (`glyph_id`,
// `h_advance_unscaled`, `units_per_em`, …) on `FontArc`. It is imported as `_`
// because this module already defines its own `pub struct Font` (a cached-font
// reference), so the trait name would otherwise collide. `as _` brings the
// trait's methods into scope without binding the name.
//
// `FontArc` is the `Arc`-backed font type; unlike `FontVec` it is `Clone`
// (cheaply), which `FontData`'s `#[derive(Clone)]` and the subset-clone path
// require.
...
impl FontCache {
    ...
    pub fn add_font(&mut self, font_data: FontData) -> Font {
        ...
        let font = Font::new(self.fonts.len(), is_builtin, &font_data.ab_font);
    ...
    /// Returns a reference to the ab_glyph font for the given font, if available.
    ///
    /// This method may only be called with [`Font`][] instances that have been created by this
    /// font cache.
    ///
    /// [`Font`]: struct.Font.html
    pub fn get_ab_font(&self, font: Font) -> &ab_glyph::FontArc {
        &self.fonts[font.idx].ab_font
    }
}

/// Parses font bytes into a cheaply-cloneable `FontArc`, selecting the first font
/// for collections.
///
/// `FontArc` has no `try_from_vec_and_index` of its own, so we parse a `FontVec`
/// (which supports a collection index) and wrap it. This natively handles both
/// single fonts and multi-font TTC collections (NotoSansCJK, macOS STHeiti,
/// Windows msyh), unlike rusttype's `into_font` which rejected collections.
/// ab_glyph also parses CFF/PostScript outlines, so OTF fonts work here too.
fn parse_font(data: Vec<u8>) -> Result<FontArc, Error> {
    let font_vec = ab_glyph::FontVec::try_from_vec_and_index(data, 0)
...
#[derive(Clone, Debug)]
pub struct FontData {
    /// The ab_glyph font used for metrics (glyph widths, kerning).
...
impl FontData {
    ...
    ///
    /// The provided data must be a readable font (TrueType or OpenType/CFF).  If `builtin` is set,
    /// a built-in PDF font is used instead of embedding the font in the PDF file (see the [module
    ...
    pub fn new(data: Vec<u8>, builtin: Option<printpdf::BuiltinFont>) -> Result<FontData, Error> {
        ...
        let ab_font = parse_font(data)?;
        if ab_font.units_per_em().is_none() {
            Err(Error::new(
    ...
    pub fn new_shared(
        ...
        let ab_font = parse_font(shared_data.to_vec())?;
        if ab_font.units_per_em().is_none() {
            Err(Error::new(
                "The font is not scalable",
                ErrorKind::InvalidFont,
    ...
    /// This avoids re-parsing the font with ab_glyph, which is expensive for large fonts.
    ///
    /// # Arguments
    /// * `source` - The FontData to clone the ab_glyph font from
    ...
    /// - Using the FULL font for metrics (glyph widths, kerning) via ab_glyph
    /// - Embedding the SUBSET font data in the PDF (smaller file size)
    /// - Mapping characters to their correct glyph IDs in the subset
    ///
    /// # Arguments
    /// * `metrics_data` - The FULL original font data (used by ab_glyph for metrics)
    ...
    pub fn new_with_subset(
        ...
        let ab_font = parse_font(metrics_data.to_vec())?;
    ...
    ///
    /// The path must point to a file that can be read as a font (TrueType or OpenType/CFF).  If
    ...
    /// given font must be metrically identical to the built-in font.
    pub fn load(
    ...
    pub fn has_glyph(&self, c: char) -> bool {
        // In ab_glyph, glyph ID 0 is the .notdef glyph (missing character indicator)
        // If the glyph for a character has ID 0, the font doesn't support it
        self.ab_font.glyph_id(c).0 != 0
    }
...
/// Horizontal glyph metrics in normalized em units (font-unit value × scale factor).
///
/// This replaces `rusttype::HMetrics` after the migration to ab_glyph, which does not
/// expose a comparable struct: ab_glyph returns unscaled advance and side-bearing as
/// separate `f32` values, so we wrap them here to preserve the original call sites.
...
pub struct Font {
    idx: usize,
    is_builtin: bool,
    /// Normalized per-font-unit scale factor: `(ascent - descent) / units_per_em`.
    /// This is the ab_glyph equivalent of rusttype's `Scale::uniform(glyph_height)`:
    /// multiply an unscaled font-unit value by this to get the value in em units.
    scale: f32,
    line_height: Mm,
    glyph_height: Mm,
    ascent: Mm,
...
impl Font {
    fn new(idx: usize, is_builtin: bool, ab_font: &ab_glyph::FontArc) -> Font {
        let units_per_em = ab_font
            .units_per_em()
            .expect("non-scalable font passed to Font::new");

        let ascent_unscaled = ab_font.ascent_unscaled();
        let descent_unscaled = ab_font.descent_unscaled();
        let line_gap_unscaled = ab_font.line_gap_unscaled();

        let glyph_height = (ascent_unscaled - descent_unscaled) / units_per_em;
        // Per-font-unit scale factor that reproduces rusttype's
        // `Scale::uniform(glyph_height)` behavior. rusttype's `.scaled(scale).h_metrics()`
        // returns `unscaled * scale / units_per_em`, so with rusttype's
        // `scale = glyph_height` the effective per-unit factor is
        // `glyph_height / units_per_em`. ab_glyph returns unscaled values, so we
        // store that factor directly and multiply by it.
        let scale = glyph_height / units_per_em;

        let ascent = ascent_unscaled / units_per_em;
        let descent = descent_unscaled / units_per_em;
        let line_height = glyph_height + line_gap_unscaled / units_per_em;
    ...
    fn char_h_metrics(&self, font_cache: &FontCache, c: char) -> HMetrics {
        ...
        if self.is_builtin {
            ...
            // ab_glyph returns unscaled advance/side-bearing in font units; apply the
            // per-font-unit scale factor, matching rusttype's `.scaled(scale).h_metrics()`.
            let font = font_cache.get_ab_font(*self);
            let id = font.glyph_id(c);
            HMetrics {
                advance_width: font.h_advance_unscaled(id) * self.scale,
                left_side_bearing: font.h_side_bearing_unscaled(id) * self.scale,
            }
        }
    }
    ...
    pub fn str_width(&self, font_cache: &FontCache, s: &str, font_size: u8) -> Mm {
        let str_width: Mm = if self.is_builtin {
            ...
            s.chars()
                .map(|c| {
                    let id = font.glyph_id(c);
                    font.h_advance_unscaled(id) * self.scale
                })
                .map(|w| Mm::from(printpdf::Pt(f32::from(w * f32::from(font_size)))))
                .sum()
        };
            ...
            iter.into_iter()
                ...
                .scan(None, |last, id| {
                    let pos = if let Some(last) = last {
                        // ab_glyph returns unscaled kerning in font units; apply the scale
                        // factor to match rusttype's `pair_kerning(scale, ...)`.
                        Some(font.kern_unscaled(*last, id) * self.scale)
                    } else {
                        Some(0.0)
                    };
                    *last = Some(id);
    ...
    /// the original glyph IDs from ab_glyph.
    ///
    /// The given [`FontCache`][] must be the font cache that loaded this font.
    ///
    /// [`FontCache`]: struct.FontCache.html
        ...
        if let Some(ref glyph_map) = font_data.glyph_id_map {
            ...
            iter.into_iter()
                .map(|c| {
                    glyph_map
                        .get(c)
                        .unwrap_or_else(|| font.glyph_id(c).0 as u16)
                })
                .collect()
        } else {
            // Original behavior for non-subset fonts
            iter.into_iter()
                .map(|c| font.glyph_id(c).0 as u16)
                .collect()
        }
    }
