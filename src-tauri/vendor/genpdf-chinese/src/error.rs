...
impl error::Error for Error {
    fn source(&self) -> Option<&(dyn error::Error + 'static)> {
        match &self.kind {
            ...
            ErrorKind::FontParseError(err) => Some(err),
...
pub enum ErrorKind {
    ...
    /// An error caused by `ab_glyph` font parsing.
    FontParseError(ab_glyph::InvalidFont),
...
impl From<ab_glyph::InvalidFont> for ErrorKind {
    fn from(error: ab_glyph::InvalidFont) -> ErrorKind {
        ErrorKind::FontParseError(error)
    }
}
