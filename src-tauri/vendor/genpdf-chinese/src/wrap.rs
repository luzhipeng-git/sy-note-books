...
/// Checks if a character is a CJK (Chinese, Japanese, Korean) ideograph.
/// This includes the main CJK Unified Ideographs blocks.
    ...
    fn next(&mut self) -> Option<style::StyledString> {
        if self.s.as_ref().map(|s| s.s.is_empty()).unwrap_or(true) {
            ...
            if let Some(s) = &self.s {
                ...
                self.cjk_mode = contains_cjk(&s.s);
            }
        }

        if let Some(s) = &mut self.s {
            if self.cjk_mode {
                // For CJK text, split character by character for proper line breaking
                // This allows text wrapping to work at character boundaries
                let mut iter = s.s.chars();
                ...
                // For non-CJK text, split at the first space or use the complete string
